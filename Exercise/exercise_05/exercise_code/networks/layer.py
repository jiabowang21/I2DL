...
class Sigmoid:
    ...
    def forward(self, x):
        ...
        # computar el sigmoid function con el parámetro x
        out = 1 / (1 + np.exp(-x))
...
class Relu:
    ...
    def forward(self, x):
        ...
        out = np.array(np.maximum(np.zeros(x.shape), x))
...
def affine_backward(dout, cache):
    ...
    x_new = np.reshape(x, (N, D)) #shape [N, D]
    
    dw = np.dot(x_new.T, dout) #shape [D, N] * [N, M] -> [D, M]
